...
cd src/opus-file
./autogen.sh
DEPS_LIBS="-L$outputFolder" DEPS_CFLAGS="-I$current/src/opus/include -I$current/src/libogg/include" ./configure --disable-http  --disable-doc --disable-examples --disable-http
make clean
make
cp ./.libs/*.so ./.libs/*.a $outputFolder/
make clean
...
./autogen.sh
DEPS_LIBS="-L$outputFolder" DEPS_CFLAGS="-I$current/src/opus/include  -I$current/src/libogg/include" ./configure  --disable-doc --disable-examples
make clean
make
cp ./.libs/*.so ./.libs/*.a $outputFolder/
make clean
